...
/**
 * Ruslan Collection Report
 *
 * Reads `docs/ruslan.csv` to get the list of device IDs associated with
 * collector Ruslan and prints a console summary of how much money those
 * devices collected for the current (or specified) calendar month.
 *
 * Usage:
 *   node reports/ruslan-reports.js            # Uses current month
 *   node reports/ruslan-reports.js --month=2025-11  # Explicit month
 */
...
const CSV_PATH = path.join(ROOT_DIR, 'docs', 'ruslan.csv');
...
async function fetchMonthlyTotals(startISO, endISO) {
    ...
    `, {
        replacements: {
            start_date: startISO,
            end_date: endISO,
            collector_nik: 'Р СѓСЃР»Р°РЅ',
            note_pattern: '%Р СѓСЃР»Р°РЅ%'
        }
    });
...
async function main() {
    ...
    console.log('🧾 Ruslan collection report');
    ...
    try {
        ...
        console.log('-------------------------------------------');
        console.log(`✅ Total collected by Ruslan’s devices: ${totalCollected.toFixed(2)} грн`);
    } catch (error) {
        logger.error('Failed to generate Ruslan report:', error);
        process.exitCode = 1;
    } finally {
