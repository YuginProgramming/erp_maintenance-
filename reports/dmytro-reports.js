...
/**
 * Dmytro Collection Report
 *
 * Reads `docs/dmytro.csv` to get the list of device IDs associated with
 * collector Dmytro and prints a summary of how much money those devices
 * collected for a calendar month (defaults to current month).
 */
...
const CSV_PATH = path.join(ROOT_DIR, 'docs', 'dmytro.csv');
...
async function fetchMonthlyTotals(startISO, endISO) {
    ...
    `, {
        replacements: {
            start_date: startISO,
            end_date: endISO,
            collector_nik: 'Р”РјРёС‚СЂРѕ',
            note_pattern: '%Р”РјРёС‚СЂРѕ%'
        }
    });
...
async function main() {
    ...
    console.log('🧾 Dmytro collection report');
    ...
    try {
        ...
        console.log('-------------------------------------------');
        console.log(`✅ Total collected by Dmytro’s devices: ${totalCollected.toFixed(2)} грн`);
    } catch (error) {
        logger.error('Failed to generate Dmytro report:', error);
        process.exitCode = 1;
    } finally {
